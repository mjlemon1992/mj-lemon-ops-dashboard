...
module.exports = (pool) => {
  ...
  // Techs hidden on the Technicians page — hiding someone there now drops them
  // from bonus too (calc + roster), so it's the single "this person is off the
  // shop" switch. bonus_person has no tech_id, and roster names are often short
  // ("Hayden") while the hidden list carries full SM names ("Hayden Ward"), so we
  // reuse the SAME fold + first-name-token match the efficiency importer uses.
  const hiddenTechNames = async (locationId) => {
    try {
  ...
  const activePeople = async (locationId) => {
    const [{ rows }, hidden] = await Promise.all([
      ...
      hiddenTechNames(locationId),
    ]);
    const isHidden = makeIsHidden(hidden);
    return rows.filter((p) => !isHidden(p.name));
  };
  ...
  router.get('/:locationId/overview', ...read, scoped, async (req, res) => {
    try {
      ...
      const _isHidden = makeIsHidden(await hiddenTechNames(locationId));
      const people = _allPeople.filter((p) => !_isHidden(p.name));
  ...
  router.put('/people/:personId', ...write, async (req, res) => {
    try {
      await ensure();
      const { name, active, efficiency_floor, role, in_bonus, vacation_hours_per_year } = req.body || {};
      ...
      if (vacation_hours_per_year != null && !(vacation_hours_per_year >= 0 && vacation_hours_per_year <= 2000)) return fail(res, 'vacation_hours_per_year must be 0–2000', 400);
      // Holiday allowance (HOURS): only touched when the key is present; null clears.
      const hasVac = Object.prototype.hasOwnProperty.call(req.body || {}, 'vacation_hours_per_year');
      if (hasVac) await pool.query('UPDATE bonus_person SET vacation_hours_per_year=$2 WHERE id=$1', [req.params.personId, vacation_hours_per_year == null ? null : Math.round(Number(vacation_hours_per_year) * 100) / 100]);
