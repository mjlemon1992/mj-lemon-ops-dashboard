...
module.exports = (pool) => {
  ...
  await pool.query("ALTER TABLE locations ADD COLUMN IF NOT EXISTS gbp_location_name VARCHAR(160)");
    ...
    // Shop-floor board rest-screen window (hours 0-23, TV-local). start===end
    // means the night screen never shows.
    await pool.query('ALTER TABLE locations ADD COLUMN IF NOT EXISTS night_start INTEGER DEFAULT 21');
    await pool.query('ALTER TABLE locations ADD COLUMN IF NOT EXISTS night_end INTEGER DEFAULT 6');
    _colInit = true;
  };
  ...
  router.put('/:id', authenticateToken, requireOwner, async (req, res) => {
    const { name, address, city, province, shopmonkey_location_id, qbo_company_id, qbo_slug, slack_channel, labour_rate, stale_threshold_days, parts_margin_target, efficiency_target, pph_target, active, display_pin, weekly_hours, display_show_leaderboard, open_days, fb_page_id, ig_user_id, gbp_location_name, night_start, night_end } = req.body;
