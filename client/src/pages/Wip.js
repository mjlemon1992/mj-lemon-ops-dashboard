...
const card = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px 18px' };
  const label = { fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.04em' };
  const big = { fontSize: '24px', fontWeight: '600', color: 'var(--text)', marginTop: '4px' };
  const sub = { fontSize: '12px', color: 'var(--text3)', marginTop: '3px' };
  const th = { padding: '8px 12px', fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', textAlign: 'left' };
  const td = { padding: '8px 12px', color: 'var(--text2)', fontSize: '13px' };
...
<div style={{ padding: '12px 12px 4px', fontSize: '13px', fontWeight: '600', color: 'var(--text)' }}>Aging chase list ({agingCount})</div>
...
<div style={{ padding: '12px 12px 4px', fontSize: '13px', fontWeight: '600', color: 'var(--text)' }}>Active pipeline ({activeCount})</div>
...
{data && data.synced_at && (<div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '10px' }}>Synced {new Date(data.synced_at).toLocaleString('en-CA')}</div>)}
  </>)}
  </div>
);
}
